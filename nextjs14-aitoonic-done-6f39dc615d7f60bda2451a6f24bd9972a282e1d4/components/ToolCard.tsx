...
export default function ToolCard({ tool, showCategory = false, categoryName }: ToolCardProps) {
  ...
  return (
    <Link
      href={`/ai/${toolSlug}`}
      className="group block rounded-xl p-[1px] bg-gradient-to-br from-primary/25 via-primary/10 to-transparent hover:from-primary/45 hover:via-primary/20 hover:to-transparent transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-primary/40"
    >
      <div className="bg-card rounded-[inherit] shadow-sm border border-border/60 hover:border-primary/40 motion-safe:transition-all motion-safe:duration-300 motion-safe:ease-out group-hover:shadow-lg group-hover:shadow-primary/10">
        <div className="p-4 flex items-start gap-4">
          {/* Logo 48x48 fixed */}
          <div className="flex-shrink-0 w-12 h-12 rounded-md overflow-hidden bg-secondary grid place-items-center motion-safe:transition-transform motion-safe:duration-300 group-hover:scale-105">
            {tool.image_url ? (
              <Image
                src={tool.image_url}
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              {/* Icon-only CTA visual indicator */}
              <svg className="w-5 h-5 text-muted-foreground flex-shrink-0 motion-safe:transition-transform group-hover:translate-x-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
